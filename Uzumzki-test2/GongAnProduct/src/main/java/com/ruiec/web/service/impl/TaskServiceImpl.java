...
public class TaskServiceImpl extends BaseServiceImpl<Task, Integer> implements TaskService {
	...
	@Override
	@Transactional
	public JsonReturn save(Task task, TaskDTO taskDTO) {
		...
		for (int i = 0; i < taskDTO.getAreaUnitIds().length; i++) {
			...
			if (unitRank.equals("city")) {
				...
				if (this.isUnitId(taskDTO.getLoginUserUnit(), sendUnit.getId())) {
					task.setStatus("1");// 任务状态
					task.setIsCheck(1);// 不需要审核
				} else {
					task.setStatus("0");// 任务状态
					task.setIsCheck(2);// 需要审核
					break;
				}
		...
		try {
			...
			for (int i = 0; i < taskDTO.getAreaUnitIds().length; i++) {
				...
				// 生成推送消息
				Unit unit = new Unit();
				unit.setId(taskDTO.getAreaUnitIds()[i]);
				// 根据单位id获取对应的管理员
				List<UserUnit> userUnits = userUnitService.findList(null, Filter.eq("unit", unit), null);
				for (int j = 0; j < userUnits.size(); j++) {
					Message message = new Message();
					message.setMessageType(3);
					message.setSourceId(task.getId());
					message.setReleaseId(taskDTO.getUser().getId());
					message.setReceiveId(userUnits.get(j).getUser().getId());
					message.setIsPush(0);
					// 需要审核的任务则等待推送
		...
		return new JsonReturn(200, "创建成功", "/admin/syntheticAnalysis/taskList.shtml");
	}
	...
	@Override
	@Transactional
	public JsonReturn saveTaskOver(Task task, TaskDTO taskDTO) {
		Task outTask = super.get(taskDTO.getTaskId());
		if (outTask.getStatus().equals("3")) {
			return new JsonReturn(400, "请不要重复操作");
		}
		// 判断当前用户是否有权限结束该任务
		if (outTask.getOriginatorUserId().equals(taskDTO.getUser().getId())) {
			task.setName(outTask.getName());// 方便记录日志的操作
			task.setId(taskDTO.getTaskId());// 任务id
			task.setStatus("3");// 改为结束状态
			task.setLog(outTask.getLog() + ";" + taskDTO.getUser().getPoliceName() + "结束了" + task.getName() + "任务");
			super.updateInclude(task, new String[] { "overReason", "status", "log" }, null);// 只修改结束理由字段
			return new JsonReturn(200, "已结束任务", "/admin/syntheticAnalysis/taskList.shtml");
		} else {
			return new JsonReturn(400, "没有权限执行该操作");
		}
	}
	...
	@Override
	@Transactional
	public JsonReturn update(Task task, TaskDTO taskDTO) {
		...
		if (taskDTO.getAreaUnitIds() == null) {
			return new JsonReturn(400, "请选择单位");
		}
		...
		return new JsonReturn(200, "编辑成功", "/admin/syntheticAnalysis/taskList.shtml");
	}
