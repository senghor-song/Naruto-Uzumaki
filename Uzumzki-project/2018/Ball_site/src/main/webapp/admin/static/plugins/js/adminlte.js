  ...
  var Treeview = function () {
    ...
    Treeview.prototype.collapse = function collapse(treeviewMenu, parentLi) {
      ...
      treeviewMenu.slideUp(this._config.animationSpeed, function () {
        ...
        treeviewMenu.find(Selector.OPEN + ' > ' + Selector.TREEVIEW_MENU).slideUp();//guan
        treeviewMenu.find(Selector.OPEN).removeClass(ClassName.OPEN);
      });
    };

    Treeview.prototype.toggle = function toggle(event) {//click
      ...
      if (isOpen) {
        this.collapse($(treeviewMenu), parentLi);
          ...
          for (var i = 0; i < $('.has-treeview').length; i++) {
              this.collapse($('.has-treeview').eq(i).find('.nav-treeview'), $('.has-treeview').eq(i));
          }
        this.expand($(treeviewMenu), parentLi);//kai
      }
    };
