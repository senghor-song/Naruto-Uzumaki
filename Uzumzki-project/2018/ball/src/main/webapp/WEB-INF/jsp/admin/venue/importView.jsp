...
<head>
    ...
    <link href="/admin/css/jquery.lineProgressbar.css" rel="stylesheet">
...
<body>
    ...
    <script src="/admin/static/js/jquery.lineProgressbar.js"></script>
<script>
    $(function () {
		//绑定页面的上传的JS
		var Myuploader = WebUploader.create({
          auto: true,   
          swf: '/admin/static/plugins/webuploader/Uploader.swf',
          server: "/admin/venue/importExcel", 
          pick: '#picker'
      	});
    		var interval = null;
  			var current = 0;
  			...
  			function increment(){
  				$.ajax({  
  			        type : "POST",  //提交方式  
  			        url : "/admin/common/redisupload?redisname=venue",//路径  
  			        ...
  			        success : function(result) {//返回数据根据结果进行相应的处理  
  			            if ( result.code == 200 ) {  
  							if(result.data.current == 100) { 
  								$.ajax({  
  	  	  		  			        type : "POST",  //提交方式  
  	  	  		  			        url : "/admin/common/deleteRedisupload?redisname=venue",//路径  
  	  	  		  			        dataType:"json",
                   ...
                   var _name =file.name; 
    	                if( _format == "xls" || _format == "xlsx" || _format == "xlt" || _format == "xlsm" || _format == "XLS" || _format == "XLSX" || _format == "XLT" || _format == "XLSM"  ){
    	                	if(data.code == 200){
    	                		...
    	                		msg += "导入成功日志:</br>" + data.data.succeedlist + "</br>";
...
<div id="ExcelUpload">
	...
	<div class="float-left mr-1">
           ...
           href="/admin/static/场馆模板.xlsx" download="场馆模板181206.xlsx">Excel规范模板下载</a>
       </div>
       <br>
       <br>
