...
public class ApiMemberController {
	...
	public ApiMessage getUserInfo(HttpServletRequest request) {
		...
		String userinfo = HttpUtils.sendGet("https://api.weixin.qq.com/cgi-bin/user/info", "access_token=" + access_token.get("access_token") + "&openid="
				+ openid + "&lang=zh_CN");
	...
	public ApiMessage addVenueHabit(HttpServletRequest request, String id) {
		...
		int flag = memberHabitMapper.insert(memberHabit);
		if (flag > 0) {
	...
	public ApiMessage deleteVenueHabit(HttpServletRequest request, String id) {
		...
		MemberHabit memberHabit = memberHabitMapper.selectByPrimaryKey(id);
		if (memberHabit.getMemberid().equals(member.getId())) {
			int flag = memberHabitMapper.deleteByPrimaryKey(id);
			if (flag > 0) {
