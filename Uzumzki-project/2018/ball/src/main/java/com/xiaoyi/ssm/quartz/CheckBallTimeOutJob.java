				...
				if (inviteBall.getToRequire() == 2 && inviteBall.getMinBoy() > inviteJoins.size()) {
					...
					for (int i = 0; i < inviteJoins.size(); i++) {
						...
						if (!StringUtil.isBank(inviteJoin.getMember().getOpenid())) {
							logger.info(WXUtil.sendWXappTemplate(inviteJoin.getMember().getOpenid(), WXConfig.templateId3, "/pages/index/index?id="
							...
							if (inviteBall.getReceiveFlag() == 1) {
								...
								logger.info(WXUtil.sendWXappTemplate(inviteJoin.getMember().getOpenid(), WXConfig.templateId2, "/pages/index/index?id="
