...
//                object = getObject(redisName, key);
	 ...
	 * @Description: 小区数据初始化
	 * @author 宋高俊  
	 * @date 2018年7月23日 上午10:31:34 
	 */ 
	public static void initEstate() {
		LOGGER.info("初始化小区缓存数据开始 ...");
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			EstateService estateService = SpringUtils.getBean("estateServiceImpl", EstateService.class);
			List<Estate> estates = estateService.selectByAll(null);
			for (Estate estate : estates) {
				if (estate != null && estate.getId() != null) {
					map.put(estate.getId(), estate);
				}
			}
			RedisUtil.setRedis(Global.REDIS_ESTATE_MAP, map);
			LOGGER.info("初始化小区缓存数据成功，共找到" + map.size() + "个小区数据");
		} catch (Exception e) {
			LOGGER.info("初始化小区缓存数据时，出现错误：" + e);
			e.printStackTrace();
		}
	}
	...
	public static void initWeb() {
		LOGGER.info("初始化网站缓存数据开始 ...");
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			WebSiteService webSiteService = SpringUtils.getBean("webSiteServiceImpl", WebSiteService.class);
			List<WebSite> webSites = webSiteService.selectByAll(null);
			for (WebSite webSite : webSites) {
				if (webSite != null && webSite.getId() != null) {
					map.put(webSite.getId(), webSite);
				}
			}
			RedisUtil.setRedis(Global.REDIS_WEB_MAP, map);
			LOGGER.info("初始化网站缓存数据成功，共找到" + map.size() + "个网站数据");
		} catch (Exception e) {
			LOGGER.info("初始化网站缓存数据时，出现错误：" + e);
			e.printStackTrace();
		}
	}
	...
	public static void initCity() {
		...
		try {
			...
			LOGGER.info("初始化城市缓存数据成功");
		} catch (Exception e) {
			LOGGER.info("初始化城市缓存数据时，出现错误：" + e);
			e.printStackTrace();
		}
	}
}