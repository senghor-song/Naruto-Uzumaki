...
<head>
	...
	<script type="text/javascript" src="/WebRelease/admin/static/js/GeoUtils.js"></script>
	<title>地图展示</title>
</head>
<body>
	<div id="allmap"></div>
	...
	<script type="text/javascript">
		...
		for (var i = 0; i < 10; i ++) {
			...
			var myIcon1 = new BMap.Icon("/WebRelease/admin/static/image/img1.png", new BMap.Size(128,128));
			var marker1 = new BMap.Marker(pt1,{icon:myIcon1});  // 创建标注
			map.addOverlay(marker1); // 将标注添加到地图中
		}
		for (var i = 0; i < 10; i ++) {
			var pt1 = new BMap.Point(sw.lng + lngSpan * (Math.random() * 0.7), ne.lat - latSpan * (Math.random() * 0.7));
			var myIcon1 = new BMap.Icon("/WebRelease/admin/static/image/img2.png", new BMap.Size(128,128));
			var marker1 = new BMap.Marker(pt1,{icon:myIcon1});  // 创建标注
			map.addOverlay(marker1); // 将标注添加到地图中
		}
		...
		for (var i = 0; i < 10; i ++) {
			var pt1 = new BMap.Point(sw.lng + lngSpan * (Math.random() * 0.7), ne.lat - latSpan * (Math.random() * 0.7));
			var myIcon1 = new BMap.Icon("/WebRelease/admin/static/image/img3.png", new BMap.Size(128,128));
			var marker1 = new BMap.Marker(pt1,{icon:myIcon1});  // 创建标注
			map.addOverlay(marker1); // 将标注添加到地图中
		}
		...
		for (var i = 0; i < 10; i ++) {
			var pt1 = new BMap.Point(sw.lng + lngSpan * (Math.random() * 0.7), ne.lat - latSpan * (Math.random() * 0.7));
			var myIcon1 = new BMap.Icon("/WebRelease/admin/static/image/img4.png", new BMap.Size(128,128));
			var marker1 = new BMap.Marker(pt1,{icon:myIcon1});  // 创建标注
			map.addOverlay(marker1); // 将标注添加到地图中
		} */
		...
		$.ajax({  
            type : "POST",  //提交方式  
            url : "/WebRelease/admin/baiduMap/index/getTopLngLat",//路径  
            ...
            success : function(result) {//返回数据根据结果进行相应的处理  
            	...
            	for (var i = 0; i < list0.length; i ++) {
            		if(!(list0[i].lng == 0 && list0[i].lat == 0)){
	        			...
	        			var myIcon1 = new BMap.Icon("/WebRelease/admin/static/image/img1.png", new BMap.Size(40,40));
            	...
            	for (var i = 0; i < list1.length; i ++) {
            		if(!(list1[i].lng == 0 && list1[i].lat == 0)){
	        			...
	        			var myIcon1 = new BMap.Icon("/WebRelease/admin/static/image/img2.png", new BMap.Size(40,40));
	        			var marker = new BMap.Marker(pt1,{icon:myIcon1});  // 创建标注
            	...
            	for (var i = 0; i < list2.length; i ++) {
            		if(!(list2[i].lng == 0 && list2[i].lat == 0)){
	        			...
	        			var myIcon1 = new BMap.Icon("/WebRelease/admin/static/image/img3.png", new BMap.Size(40,40));
	        			var marker = new BMap.Marker(pt1,{icon:myIcon1});  // 创建标注
            	...
            	for (var i = 0; i < list3.length; i ++) {
            		if(!(list3[i].lng == 0 && list3[i].lat == 0)){
	        			...
	        			var myIcon1 = new BMap.Icon("/WebRelease/admin/static/image/img4.png", new BMap.Size(40,40));
	        			var marker = new BMap.Marker(pt1,{icon:myIcon1});  // 创建标注
		...
		function show(id){
			$.ajax({  
	            type : "POST",  //提交方式  
	            url : "/WebRelease/admin/baiduMap/index/getTopLngLatById",//路径  
	            ...
	            success : function(result) {//返回数据根据结果进行相应的处理  
	            	...
	            	for (var i = 0; i < list.length; i ++) {
	            		if(!(list[i].lng == 0 && list[i].lat == 0)){
		        			...
		        			var myIcon1 = new BMap.Icon("/WebRelease/admin/static/image/img"+id+".png", new BMap.Size(40,40));
		        			var marker1 = new BMap.Marker(pt1,{icon:myIcon1});  // 创建标注
