...
$.fn.extend({
	...
	initPage : function(totalPage,currentPage,pageSize,targetForm) {
		...
		var pageHtml = "第"+currentPage+"页，共"+totalPage+"页，每页"+pageSize+"条 ";
		/**
		 * 当前是第一页，上一页则是灰的
		 */
		pageHtml += '<ul class="am-pagination tpl-pagination">';
		if (currentPage <= 1) {
			pageHtml += '<li class="am-disabled"><a class="no" href="javascript:;">上一页</a></li>';
		} else {
			pageHtml += '<li><a href="javascript:$.fn.goPage('+(currentPage-1)+');">上一页</a></li>';
		}
		if (totalPage <= limitSize) {
			for (var i = 1; i <= totalPage; i++) {
				...
				pageHtml += '><a href="javascript:$.fn.goPage('+i+');">' + i + '</a></li>';
			}
		...
		if (currentPage == totalPage) {
			pageHtml += '<li class="am-disabled"><a class="no" href="javascript:;">下一页</a></li>';
		} else {
			pageHtml += '<li><a href="javascript:$.fn.goPage('+(currentPage+1)+');">下一页</a></li>';
		}
		pageHtml += ' <li style="border:none;color:#333;" class="return_other"><span style="float:left;border:none;">转到：</span><cite class="wsc-form-cite a text"><input type="text" id="page" style="width:70px;height:26px;float:left;" name="page" value="" inited="true" ></cite><span style="float:left;border:none;">页</span></li>';
		pageHtml += '<li><a href="javascript:;" class="_jump_page_link">确定</a></li>';
		pageHtml += '</ul>';
		if(totalPage>1){
			$(this).html(pageHtml);
		}
	...
	goPage:function(nextPage){
		if(!(/^\d+$/.test(nextPage)) || parseInt(nextPage)<=0 || parseInt(nextPage)>$.fn.totalPage){
			layer.msg("请输入正确的跳转页数");
			return;
		}
		if($.fn.targetForm.length){
			if($('[name="currentPage"]').length==0){
				$("<input type='hidden' name='currentPage' value='"+nextPage+"' />").appendTo($.fn.targetForm);
			}else{
				$('[name="currentPage"]').eq(0).attr("value",nextPage);
			}
			$.fn.targetForm.submit();
		}else{
			var _pageUrl = window.location.href;
			var params = _pageUrl.split("?");
			window.location.href = params[0]+"?currentPage="+nextPage;
		}
	},
	/**
	 * ajax分页回调
	...
	buildPage:function(pageNum,pageSize,total){
		...
		pageHtml += '<ul class="am-pagination tpl-pagination">';
		if (currentPage <= 1) {
			pageHtml += '<li><a class="no" href="javascript:;">上一页</a></li>';
		} else {
			pageHtml += '<li><a href="javascript:$.fn.goAjaxPage('+(currentPage-1)+');">上一页</a></li>';
		}
		if (totalPage <= limitSize) {
			for (var i = 1; i <= totalPage; i++) {
				pageHtml += '<li><a ';
				...
				pageHtml += '  href="javascript:$.fn.goAjaxPage('+i+');">' + i + '</a></li>';
			}
		}else if(currentPage <= limitSize / 2 + 1){
			for (var i = 1; i <= limitSize; i++) {
				pageHtml += '<li><a ';
		 ...
		 */
		if (currentPage == totalPage) {
			pageHtml += '<li><a class="no" href="javascript:;">下一页</a></li>';
		} else {
			pageHtml += '<li><a href="javascript:$.fn.goAjaxPage('+(currentPage+1)+');">下一页</a></li>';
		}
		pageHtml += ' <li style="border:none;color:#333;" class="return_other"><span style="float:left;border:none;">转到：</span><cite class="wsc-form-cite a text"><input type="text" id="page" style="width:70px;height:26px;float:left;" name="page" value="" inited="true" ></cite><span style="float:left;border:none;">页</span></li>';
		pageHtml += '<li><a href="javascript:;" class="_jump_page_link">确定</a></li>';
		pageHtml += '</ul>';
