	...
	rui_validate.prototype = {
		...
		*/
		initload:function(){ 
			...
			* @time 2015年8月20日 10:07:09
			*/
			$("body").on("keyup","*[verify='isNumb'] , *[verify='isPhone'] , *[verify='isBankCard']",function(event){
			...
			*/ 
			$("*[placeholder]").each(function() { 
				...
				var _theBrowser = window.navigator.userAgent.toLowerCase() ;	
				//console.log(_theBrowser);		
				/*if(!Array.indexOf){
		...
		check_objInfo: function (obj, rui_Validate_options) {
		    ...
		    switch (Verify) {
		        ...
		        case "isZhName":
		        	...
		        	if (!reg.test(theValue)) {
		                result = { statue: "Error_validate", obj: obj, msg: "请输入中文名" };
		            } else {
		                result = { statue: "Correct_validate", obj: obj, msg: "通过验证" };
		            }
		        	break; 	
		        ...
		        case "isBankCard":				//是银行卡号
		            ...
		            break;
		        case "isPwd":				//密码
		            var reg = /^(?!([^A-Za-z]|\d)+$)[a-zA-Z\d]{6,32}$/;  //请输入字母和数字组合的6到32位字符串
		            if (!reg.test(theValue)) { 
		                result = { statue: "Error_validate", obj: obj, msg: "请输入字母和数字组合的6到32位字符串" };
		            } else {
		                result = { statue: "Correct_validate", obj: obj, msg: "通过验证" };
                    }
		            break;
		        ...
		        case "isWX":				//是微信号码
		            ...
		            if (!reg.test(theValue)) {
		                result = { statue: 'Error_validate', obj: obj, msg: "微信账号仅支持6-20个字母、数字、下划线或减号，以字母开头" };
		            } else {
		                result = { statue: "Correct_validate", obj: obj, msg: "通过验证" };
		            }
		            break;    
		        ...
		        case "isEndTime":			//结束时间
		            ...
		            break;
		        case "isChecked":			//复选框 
					var _StartTime = rui_Validate_options.FormObj.find("*[verify='isChecked']").eq(0).val();
		            if (_StartTime != undefined && _StartTime != "") {
		                var Time_guize = this.compareTime(_StartTime, theValue, "-");
		                if (!Time_guize) { 
		                    result = { statue: "Error_validate", obj: obj, msg: "开始时间不得大于结束时间" };
		    ...
		    switch (rui_Validate_options.ShowTip) {
		        ...
		        case "Text": //文字提示 
		            ...
		            if (_theTs_msg_len < 1) {
		                ...
		                if (state == "Error_validate") {
		                	obj.removeClass("Correct_validate").addClass("Error_validate");
		                    theLineObj.find("." + rui_Validate_options.ShowTipClass).removeClass("Correct_validate").addClass("Error_validate").empty().show().text(msg);
		                } else {
		                	obj.removeClass("Error_validate").addClass("Correct_validate");
		                    theLineObj.find("." + rui_Validate_options.ShowTipClass).removeClass("Error_validate").addClass("Correct_validate").empty().text(msg);
		                }
		...
		ShowTips_Bubble: function (rui_Validate_options, obj, msg, state) {
		    ...
		    if (_obj_p.find("." + rui_Validate_options.FormIdName + "_Bubble_ts_msg").length > 0) {  //是否已有提示信息 
		        ...
		        if (state == "Error_validate") {
		            ...
		            var _style = " position: absolute;width:auto; max-width:" + (_obj_width - 15) + "px;  background-color:#fff; padding:0px 15px;height:" + (_new_height - 2) + "px;line-height:" + (_new_height - 2) + "px;border:1px solid #ddd; margin-left:" + (_obj_width + 35) + "px; float:left; box-shadow:2px 2px 2px #ddd; background-color:#fff; border-radius:3px;z-index:2;";
        ...
        */ 
		ShowTips_IconText: function (rui_Validate_options, obj, msg, state) {
		    ...
		    if (_obj_p.find("." + rui_Validate_options.FormIdName + "_Bubble_ts_msg").length > 0) {
		        ...
		        var _style = "width:auto; max-width:" + (_obj_width - 15) + "px; height:" + (_obj_height -2) + "px;line-height:" + (_obj_height - 2) + "px;  margin-left:5px; float:left; z-index:2;";
        ...
        */
		ShowTips_Icon: function (rui_Validate_options, obj, msg, state) {
		    ...
		    if (_obj_p.find("." + rui_Validate_options.FormIdName + "_Bubble_ts_msg").length > 0) {
		        ...
		        var _style = "position: absolute;width:auto; max-width:" + (_obj_width - 15) + "px; height:" + (_new_height - 2) + "px;line-height:" + (_new_height - 2) + "px;  margin-left:" + (_obj_width + 5) + "px; float:left;  margin-top:-" + _obj_height + "px; z-index:2;";
		...
		checkVerify: function (Verify, theValue, obj, rui_Validate_options) {
		    ...
		    switch (Verify) {
		        ...
		        case "isPwd":				//密码
		            var reg = /^(?!([^A-Za-z]|\d)+$)[a-zA-Z\d]{6,32}$/;  //请输入字母和数字组合的6到32位字符串
		            if (!reg.test(theValue)) {
		                result = { statue: false, obj: obj, msg: "请输入字母和数字组合的6到32位字符串" };
		            }
		            break;
		        case "isRPwd":				//重复密码 
		            var reg = /^(?!([^A-Za-z]|\d)+$)[a-zA-Z\d]{6,32}$/;  //请输入字母和数字组合的6到32位字符串
